...
class CollectionsBusiness():
    ...
    def search(cls, collections, bbox, cloud=False, time=False, limit=100):
        ...
        for cp in collections.split(','):
            ...
            full_query = 'bbox={}{}'.format(('[{}]'.format(bbox) if provider == 'DEVELOPMENT_SEED_STAC' else bbox), query)
            response = CollectionsServices.search_items(ProvidersBusiness.get_providers()[provider], collection_id, full_query)

            # get all features
            if int(limit) <= 1000 or not response.get('meta') or int(response['meta']['found']) <= 1000:
                if response:
